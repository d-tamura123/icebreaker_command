...
namespace gm {
    ...
    //   ・島衝突時: 通常の操舵を一時的に止め、後退(バック)する例外処理
    //     (SPEED_LEVELSの負の段階を使う。NPC専用の特殊な動きは増やさない)
    //   ・進捗管理: 経路上の進捗(累積距離)のハイウォーターマークを追跡し、
    //     一定時間更新が無ければ最寄りの経路上地点へワープ+向き補正する保険。
    //     流氷への衝突検知でリセットされる(暫定。onCollisionEnter()内のTODO参照。
    //     ダメージ/HPシステム実装後に見直し予定)。この計測とワープ判定はupdate()直下で
    //     常に行い、島衝突バック中かどうかによらず機能する
    //     (バック中だけ計測が止まると、最終防衛ラインとして機能しなくなるため。実機検証で判明)
    //   ・終点(G)到達でデスポーン(isAlive()がfalseになる)
    // ------------------------------------------------------------
    ...
    public:
        ...
        // 衝突検出イベント(島にぶつかった場合だけ、一時的なバック挙動に入る)
        ...
        //
        // #ifdef _DEBUG: これらは「呼び出し口」なので、リリースビルドの成果物に含めない。
        // debugForcedBadSteering_フラグ自体や、それを見るupdateSteeringAI()内のif文は
        // あえて残している(呼び出し口さえ塞げば常にfalseの無害な分岐になるため)。
#ifdef _DEBUG
