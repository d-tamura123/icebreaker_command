...
#define DXE_WATER_PLANE_SHADER_COMPLIE 0
	...
	void WaterPlane::initialize() {
		...
		hr = D3DCompileFromFile(L"resource/shader/water_plane/water_plane.hlsl", nullptr, D3D_COMPILE_STANDARD_FILE_INCLUDE,
	...
	void WaterPlane::setWaveDecentralization(const float& decent) {
		...
		wave_directionA_.x = tnl::GetRandomDistribution<float>(-r, r);
		wave_directionA_.y = tnl::GetRandomDistribution<float>(-r, r);
		wave_directionA_.z = tnl::GetRandomDistribution<float>(-r, r);
		wave_directionA_.w = tnl::GetRandomDistribution<float>(-r, r);

		wave_directionB_.x = tnl::GetRandomDistribution<float>(-r, r);
		wave_directionB_.y = tnl::GetRandomDistribution<float>(-r, r);
		wave_directionB_.z = tnl::GetRandomDistribution<float>(-r, r);
		wave_directionB_.w = tnl::GetRandomDistribution<float>(-r, r);

		wave_directionC_.x = tnl::GetRandomDistribution<float>(-r, r);
		wave_directionC_.y = tnl::GetRandomDistribution<float>(-r, r);
		wave_directionC_.z = tnl::GetRandomDistribution<float>(-r, r);
		wave_directionC_.w = tnl::GetRandomDistribution<float>(-r, r);

		wave_directionD_.x = tnl::GetRandomDistribution<float>(-r, r);
		wave_directionD_.y = tnl::GetRandomDistribution<float>(-r, r);
		wave_directionD_.z = tnl::GetRandomDistribution<float>(-r, r);
		wave_directionD_.w = tnl::GetRandomDistribution<float>(-r, r);
	...
	void WaterPlane::render(const Shared<dxe::Camera>& camera) {
		...
		sb1.cam_pos_ = camera->getPosition();
		...
		pImmediateContext->OMSetDepthStencilState(nullptr, 0);   // デフォルト＝Write ON, Test ON
		pImmediateContext->RSSetState(nullptr);                  // デフォルト＝CullBack, Solid
		pImmediateContext->OMSetBlendState(nullptr, nullptr, 0xffffffff); // デフォルト＝ブレンドなし
		*/
		// ↑後始末
	}
